    ...
    private ArrayList matchable(ArrayList<String> theAntecedents, int n, ArrayList bindings) {
        if (n == theAntecedents.size()) {
            ...
            for (int i = 0; i < assertions.size(); i++) {
                ...
                if ((new Matcher()).matching((String) theAntecedents.get(n), (String) assertions.get(i), binding)) {
            ...
            for (int i = 0; i < bindings.size(); i++) {
                for (int j = 0; j < assertions.size(); j++) {
                    if ((new Matcher()).matching((String) theAntecedents.get(n), (String) assertions.get(j),
 ...
 *
 */
public class ForwardGraphDraw extends JPanel implements Scrollable {
    String fileName;
    String workingMemoryFileName;
    FileReader f;
    StreamTokenizer st;
    WorkingMemory wm;
    ...
    ForwardGraphDraw() {
        ...
        workingMemoryFileName = "../OriginalWm.data";

        wm = new WorkingMemory();
        try {
            BufferedReader br = new BufferedReader(new FileReader(workingMemoryFileName));
                        ...
                        if (!wm.contains(newAssertion)) {
                            ...
                            if (previous_top_margin != 0) {
                                drawRightAngleArrow(g,
                                        ...
                                        top_margin - height_between_blocks
                                                - (s2.split(",").length + 2) * f.getHeight() / 2 + 2);
                            }
    ...
    public Dimension getPreferredSize() {
      ...
      public Dimension getPreferredScrollableViewportSize() {
        ...
        if (r * c < 20)
          ++r;
        return new Dimension(c * 50, r * 50 *5 );
      }
